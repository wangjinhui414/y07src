...
void fun_iso_9141_send_times( uint8 cmdid, uint8 u8FrameLen, void **pCommStatus )
{
    ...
    do
    {
        ...
        //ecu超时时间
        pfOSdelay( 0, 0, pstIsoKlineCommLib->u16TimeOut / 1000, pstIsoKlineCommLib->u16TimeOut % 1000 );
        //这里是预防ECU没有响应
        ...
        *pCommStatus = ( void * )&g_enumFrameFlgArr[2]; //当做接收正常
        ...
        //设置帧长只是取有效数据长度
        g_stDataFromEcu.u8Len = g_stUsart3Iso14230ReceiveVar.u8ReceiveCounter - 4;
        //重新使用该变量,重新将数据移动一次
