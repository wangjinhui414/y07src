...
void fun_iso_15765_send_times( uint8 u8CmdId, void **pCommStatus )
{
    ...
    do
    {
        ...
        *pCommStatus = OSMboxPend( g_pstEcuToPerformer,
            ...
            case  OS_ERR_TIMEOUT:
                {
                    ...
                    if( g_stCAN115765ReceiveVar.bFrameError )
                    {
...
void fun_iso_15765_send_cmd( uint8 u8CmdId )
{
    ...
    g_stCAN115765ReceiveVar.u8FrameTotal = 0;
    g_stCAN115765ReceiveVar.bFrameError = FALSE;
    g_stCAN115765ReceiveVar.bFrameLengTh = FALSE;
    g_stCAN115765ReceiveVar.u8FrameCounter = 0;
    //装载命令
    memcpy
    (
        g_stTxMessage.Data,
        ( ( g_pModeLibTab->pCmdTabChoice )[u8CmdId] ).pu8Command ,
        8
    );
    //发送命令
    u32FrameId = g_unnu32FrameIdLib[( ( g_pModeLibTab->pCmdTabChoice )[u8CmdId] ).u8CmdIdOffset].u32Bit;
    //滤波器设置ID
    u8Filterpid = ( ( g_pModeLibTab->pCmdTabChoice )[u8CmdId] ).u8FilterIdOffset;
    //滤波器组选择
    u8FilterGroup = ( ( g_pModeLibTab->pCmdTabChoice )[u8CmdId] ).u8FilterGroup;
    //先禁止CAN接收中断
    CAN_ITConfig( CAN_COMMUNICATION_PORT, CAN_IT_FMP0, DISABLE );
    //配置好CAN滤波器
    fun_can1_Filter_init( u8FilterGroup, g_unnu32FilterId[u8Filterpid].u32Bit );
    //开启can接收中断
    ...
    //这里也是根据所填写的长度来决定的
    g_stTxMessage.DLC = ( ( g_pModeLibTab->pCmdTabChoice )[u8CmdId] ).u8CmdLen;
    CAN_Transmit( CAN_COMMUNICATION_PORT, &g_stTxMessage );
}
...
void*pfun_iso_15765_check( void*pArg0, void*pArg1 )
{
    ...
    switch( g_stCAN115765ReceiveVar.RxMessage.Data[0] & 0xf0 )
    {
        //这里根据汽车发送过来的数据进行归类存放
        ...
        case 0x10:
            {
                ...
                //设置好帧长
                g_stDataFromEcu.u16Len = g_stCAN115765ReceiveVar.RxMessage.Data[1];
                //同时告知已经找到帧长,主要是为21帧做准备
                g_stCAN115765ReceiveVar.bFrameLengTh = TRUE;
                //存储10帧,只是存储6个字节
                memcpy( g_stDataFromEcu.u8Buf, &g_stCAN115765ReceiveVar.RxMessage.Data[2], 6 );
                //记住当前存储的数据长度
                g_stCAN115765ReceiveVar.u16DataCounter = 6;
                //发送公共帧,暂时在这里发送, 如果以后不合适可以考虑变化位置
                memcpy( &g_stTxMessage.Data, u8publicFrame, 8 );
                CAN_Transmit( CAN_COMMUNICATION_PORT, &g_stTxMessage );
                //因为有可能接收到10帧之后不给回数据,导致通讯报告错误
                g_stCAN115765ReceiveVar.bFrameError = TRUE ;
                //为接收2X帧设置计数器
                g_stCAN115765ReceiveVar.u8FrameCounter = 1;
                return ( ( void * )0 );
            }
        ...
        case 0x20:
            {
                ...
                //判断是否找到帧长
                if( !g_stCAN115765ReceiveVar.bFrameLengTh )
                {
                    return ( ( void * )0 );
                }

                //判断是不是想要接收的帧 -屏蔽了，这样只能接受15帧
//                if( g_stCAN115765ReceiveVar.u8FrameCounter != ( uint8 )( g_stCAN115765ReceiveVar.RxMessage.Data[0] & 0x0f ) )
//                {
//                    return ( ( void * )0 );
//                }

                //为下次接收2X帧做好准备
                g_stCAN115765ReceiveVar.u8FrameCounter++;
                //存储2X帧,只是存储7个字节
                memcpy( g_stDataFromEcu.u8Buf + g_stCAN115765ReceiveVar.u16DataCounter,
                        &g_stCAN115765ReceiveVar.RxMessage.Data[1],
                        7
                      );
                //设置好偏移地址为下一次做好准备
                g_stCAN115765ReceiveVar.u16DataCounter = g_stCAN115765ReceiveVar.u16DataCounter + 7;
                //判断接收到的数据是否正确
                u8DataLen =
                    g_stDataFromEcu.u16Len +
                    ( 7 - ( ( ( g_stDataFromEcu.u16Len - 6 ) % 7 ) ? ( ( g_stDataFromEcu.u16Len - 6 ) % 7 ) : 7 ) );

                //判断接收到的数据是否已经达到有效字节
                if( u8DataLen != g_stCAN115765ReceiveVar.u16DataCounter )
                {
        ...
        default :
            {
                ...
                g_stDataFromEcu.u16Len = ( g_stCAN115765ReceiveVar.RxMessage.Data[0] & 0x07 );
                //存储10帧,只是存储有效长度个字节
                memcpy( g_stDataFromEcu.u8Buf, &g_stCAN115765ReceiveVar.RxMessage.Data[1], g_stDataFromEcu.u16Len );
        ...
        case 0x10:
            {
                ...
                //发送命令
                uint32 u32FrameId = g_unnu32FrameIdLib[( g_pModeLibTab->u8ProtocolLibOffset )[4]].u32Bit;
                //设置好帧长
                g_stDataFromEcu.u16Len = g_stCAN115765ReceiveVar.RxMessage.Data[1];
                //同时告知已经找到帧长,主要是为21帧做准备
                g_stCAN115765ReceiveVar.bFrameLengTh = TRUE;
